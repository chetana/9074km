...
export const GET: RequestHandler = async (event) => {
  ...
  try {
    const [contents] = await getGcsBucket().file(`${y}/${m}/${d}/reactions.json`).download()
...
export const POST: RequestHandler = async (event) => {
  ...
  await getGcsBucket().file(`${y}/${m}/${d}/reactions.json`).save(JSON.stringify(body), { contentType: 'application/json' })
  return json({ ok: true })
}
